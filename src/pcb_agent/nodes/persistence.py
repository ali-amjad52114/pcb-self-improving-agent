...
def make_store_experience(
    ...
    def store_experience(state: AgentState) -> dict[str, Any]:
        ...
        lesson_payload = {
            "lesson_id": f"{experiment_id}_lesson",
            "run_id": run_id,
            "experiment_id": experiment_id,
        ...
        lesson_ok = False
        commit = getattr(deps.memory, "commit_experience", None)
        if callable(commit):
            try:
                commit(experiment_payload, lesson_payload)
                experiment_stored = True
                lesson_ok = True
            except Exception as exc:
                warnings.append(f"commit_experience_failed: {exc}")
