...
class ExternalMemoryAdapter:
    ...
    def commit_experience(self, experiment: dict, lesson: dict) -> dict[str, str]:
        commit = getattr(self._module, "commit_experience", None)
        if callable(commit):
            return commit(experiment, lesson)
        return {
            "experiment_id": self.store_experiment(experiment),
            "lesson_id": self.store_lesson(lesson),
        }
